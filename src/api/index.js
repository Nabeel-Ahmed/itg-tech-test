...
async function getVehicleDetails(vehicleList) {
  ...
  const promises = vehicleList.map(async (vehicle) => {
    try {
      ...
      // Could use throw error but that would break the app, console.error
      // would be good for monitoring/logs
...
function combineDetails(vehicleList, detailedVehicleList) {
  const combinedList = vehicleList.map((vehicle) => {
    const detailedVehicle = detailedVehicleList.find((detailed) => detailed.id === vehicle.id);
...
export default async function getData() {
  ...
  const combinedList = combineDetails(vehicleUrlResults, detailedVehicleArray);
